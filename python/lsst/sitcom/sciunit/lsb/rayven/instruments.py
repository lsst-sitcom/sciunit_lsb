...
class CBP:
    ...
    def calculate_position(self, angle_deg, radius=12.4, z=13.13):  # radius & z in m, angle in deg
        ...
        self.y = -radius * np.sin(
            np.deg2rad(angle_deg)
        )  # Azimuth is in a left handed coord system, (x,y,z) is right handed
        self.z = z

    def get_batoid_model(self, rotTelPos=0):
        cbp = batoid.Optic.fromYaml("CBP.yaml")
        # Need to rotate the fiducial model, which points upwards, to point dwn
        cbp = cbp.withLocalRotation(batoid.RotX(np.deg2rad(180)))
        # Rotate 120 degrees to make cbpAz=0 point directly at Simonyi
...
class LSST:
    ...
    def get_batoid_model(self, rotTelPos=0, w_pupil=False):
        ...
        m1VertexHeight = 3.53  # When Zenith pointing
        ...
        if w_pupil:
            ...
            lsst = lsst.withInsertedOptic(
                ...
                item=batoid.Interface(
                    ...
                    coordSys=batoid.CoordSys(origin=[0, 0, 0.4393899101271662]),
                ),
            )
