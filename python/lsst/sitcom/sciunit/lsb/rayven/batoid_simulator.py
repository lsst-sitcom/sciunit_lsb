...
class BatoidSimulator:
    ...
    def _validate_star_table(self, table):
        if not isinstance(table, QTable):
            raise TypeError("star table must be an astropy.table.QTable object")
        ...
        if not required_cols.issubset(table.colnames):
            ...
            raise ValueError(
                f"camera geometry coordinate transform table is missing required column(s): {missing_txt}"
            )
    ...
    def set_optic_reflectance(self):
        ...
        for surface in self.telescope.itemDict.values():
            ...
            if surface.name in self.reflectance.values.keys() and isinstance(
                surface, batoid.RefractiveInterface
            ):
                surface.forwardCoating = batoid.SimpleCoating(
                    self.reflectance.values[surface.name], 1 - self.reflectance.values[surface.name]
                )
    ...
    def set_detector_reflectance(self, dettype):
        ...
        for surface in self.telescope.itemDict.values():
            ...
            if isinstance(surface, batoid.Detector) and "Detector" in surface.name:
                surface.forwardCoating = batoid.SimpleCoating(
                    self.reflectance.values[dettype], 1 - self.reflectance.values[dettype]
                )

    def simulate_single_star(self, star_index=None, scaling=None, fa_x=None, fa_y=None, dettype=None):

        # option to provide just the star index and then retrieving everything
        # from the star_table
        ...
        rays = batoid.RayVector.asPolar(
            optic=self.telescope, wavelength=wavelength, theta_x=(fa_x), theta_y=(fa_y), nrad=300, naz=2000
        )
        ...
        for ray in rForward:
            ...
            normed_flux = self.normalize_flux(flux=ray.flux, total_flux=tot_flux, scaling=scaling)
            flux_arr.append(normed_flux)
            # label each ghost
            lbl = self.label_ghost(ray)
            labels.append(lbl)
    ...
    def store_simulated_data(self, label_arr, ray_arr, x_arr, y_arr, flux_arr, tot_flux):
        ...
        for lbl, r, x, y, f in zip(label_arr, ray_arr, x_arr, y_arr, flux_arr):
            ghost = Ghost(name=lbl, ray=r, x=x * 1e3, y=y * 1e3, flux=f)  # 1e3 to convert from m to mm
