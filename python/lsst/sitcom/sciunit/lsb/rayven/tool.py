...
class GhostTool:
    ...
    def __init__(self, obs_params=None, reflectance=None, bright_star_catalog=None, **kwargs):
    ...
    def get_data_product(self, dataset_name, detector=94):
        ...
        try:
            assert self.butler is not None, "Butler must be initialized to retrieve data products."
    ...
    def make_star_table(self, bright_star_catalog, camera_geometry):
        ...
        if not len(bright_star_catalog.table) == len(camera_geometry.coord_transform_table):
            raise ValueError(
                ...
                "camera_geometry.coord_transform_table has {len(camera_geometry.coord_transform_table)} rows."
            )
