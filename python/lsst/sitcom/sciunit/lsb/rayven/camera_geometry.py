...
class CameraGeometry:
    ...
    def __init__(self, camera, bright_star_catalog, wcs=None, coord_transform_table=None):
        ...
        if coord_transform_table is not None:
            ...
            self.coord_transform_table = self.make_coord_transform_table(
                coord_transform_dict=coord_transform_dict
            )
        else:
    ...
    def get_focal_plane_extent(self):
        ...
        self.fp_geom["width"], self.fp_geom["height"] = (
            focal_plane_bbox.getWidth(),
            focal_plane_bbox.getHeight(),
        )
    ...
    def get_star_coordinate_transforms(self):
        ...
        assert self.wcs is not None, "WCS must be provided to compute coordinate transforms."
        px_x, px_y = self.wcs.skyToPixelArray(ra, dec, degrees=True)
    ...
    def _validate_coord_transform_table(self, table):
        if not isinstance(table, QTable):
            raise TypeError(
                "camera geometry coordinate transform table must be an astropy.table.QTable object"
            )
